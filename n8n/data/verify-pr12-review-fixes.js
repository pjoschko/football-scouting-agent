...
function assertAsyncResearchFanOut(parent, child) {
  ...
  assert(execute.type === 'n8n-nodes-base.executeWorkflow', `${parent.name}: ${executeName} muss Execute Sub-workflow sein.`);
  ...
  assert(execute.parameters.mode === 'once', `${parent.name}: ${executeName} muss pro Dispatch genau ein Item mit mode="once" starten.`);
  ...
  assert(pollWait.type === 'n8n-nodes-base.wait' && pollWait.parameters.resume === 'timeInterval',
  ...
  assert(childWait.type === 'n8n-nodes-base.wait' && childWait.parameters.resume === 'timeInterval' && Number(childWait.parameters.amount) >= 1,
    `${child.name}: Dummy-Research braucht sichtbare Latenz, damit parallele Child-Executions im UAT zeitlich ueberlappen.`);
  assert(childResearch.onError === 'continueErrorOutput', `${child.name}: kandidatenspezifische Research-Fehler muessen im Child normalisiert werden.`);
...
function assertEncapsulatedCapabilityErrors() {
  const main = loadWorkflow('ai-sporting-director.json');
  const removedMainGates = [
    'Team analysieren gueltig?',
    'Scouting Brief gueltig?',
    'Kandidaten suchen gueltig?',
    'Due Diligence gueltig?',
    'Empfehlung gueltig?',
    'Ergebnis aufbereiten gueltig?',
  ];
  for (const gate of removedMainGates) {
    assert(!main.nodes.some((node) => node.name === gate),
      `${main.name}: fachliches Gate "${gate}" darf nicht mehr im Hauptworkflow liegen.`);
    assert(!main.connections[gate], `${main.name}: Connection-Block fuer entferntes Gate "${gate}" ist noch vorhanden.`);
  }

  const sharedErrorNode = 'Capability-Fehler normalisieren';
  const errorAdapter = nodeByName(main, sharedErrorNode);
  assert(errorAdapter.type === 'n8n-nodes-base.code', `${main.name}: gemeinsamer Capability-Fehleradapter muss ein Code-Node sein.`);
  assert(targets(main, sharedErrorNode, 0).some((t) => t.node === 'Fehler anzeigen'),
    `${main.name}: gemeinsamer Capability-Fehleradapter muss in Fehler anzeigen fuehren.`);

  const capabilityCalls = [
    ['Execute Workflow: Team analysieren', 'Execute Workflow: Scouting Brief erstellen'],
    ['Execute Workflow: Scouting Brief erstellen', 'Scouting Brief zur Freigabe vorlegen'],
    ['Execute Workflow: Kandidaten suchen', 'Execute Workflow: Due Diligence'],
    ['Execute Workflow: Due Diligence', 'Execute Workflow: Empfehlung erstellen'],
    ['Execute Workflow: Empfehlung erstellen', 'Execute Workflow: Ergebnis aufbereiten'],
    ['Execute Workflow: Ergebnis aufbereiten', 'Ergebnis anzeigen'],
  ];
  for (const [executeName, successTarget] of capabilityCalls) {
    const execute = nodeByName(main, executeName);
    assert(execute.type === 'n8n-nodes-base.executeWorkflow', `${main.name}: ${executeName} muss Execute Sub-workflow sein.`);
    assert(execute.typeVersion >= 1.3, `${main.name}: ${executeName} braucht typeVersion >= 1.3 fuer den stabilen Error-Pin.`);
    assert(execute.onError === 'continueErrorOutput', `${main.name}: ${executeName} muss den Error-Pin aktivieren.`);
    assert(targets(main, executeName, 0).some((t) => t.node === successTarget),
      `${main.name}: Erfolg von ${executeName} muss direkt in ${successTarget} fuehren.`);
    assert(targets(main, executeName, 1).some((t) => t.node === sharedErrorNode),
      `${main.name}: Fehler von ${executeName} muss in den gemeinsamen Capability-Fehleradapter fuehren.`);
  }

  // Process-level decisions intentionally stay in the parent workflow.
  for (const processGate of ['Eingabe validieren', 'Review-Entscheidung gueltig?', 'Review: Approve?', 'Review: Request Changes?']) {
    nodeByName(main, processGate);
  }

  const exits = [
    ['team-analysieren-subworkflow.json', 'Team analysieren: Ergebnis', '[TEAM_ANALYSIS]'],
    ['scouting-brief-subworkflow.json', 'Scouting Brief erstellen: Ergebnis', '[SCOUTING_BRIEF]'],
    ['kandidaten-suchen-subworkflow.json', 'Kandidaten suchen: Ergebnis', '[CANDIDATE_SEARCH]'],
    ['due-diligence-subworkflow.json', 'Due Diligence: Ergebnis', '[DUE_DILIGENCE]'],
    ['empfehlung-erstellen-subworkflow.json', 'Empfehlung erstellen: Ergebnis', '[RECOMMENDATION]'],
    ['ergebnis-aufbereiten-subworkflow.json', 'Ergebnis aufbereiten: Ergebnis', '[RESULT_PRESENTATION]'],
  ];

  for (const [file, exitName, prefix] of exits) {
    const workflow = loadWorkflow(file);
    const exit = nodeByName(workflow, exitName);
    assert(exit.type === 'n8n-nodes-base.code' && exit.typeVersion === 2,
      `${workflow.name}: ${exitName} muss der Code-basierte Capability-Exit sein.`);
    const code = exit.parameters.jsCode || '';
    assert(code.includes('item.valid === false') && code.includes('throw new Error') && code.includes(prefix),
      `${workflow.name}: ${exitName} muss valid=false intern mit Phasenpraefix werfen.`);
    assert(code.includes("status: 'success'"), `${workflow.name}: ${exitName} muss Erfolg explizit als success zurueckgeben.`);

    let thrown = null;
    try {
      vm.runInNewContext(`(function () { ${code} })()`, {
        $input: { item: { json: { valid: false, errorMessage: 'test-fehler' } } },
        Error,
      });
    } catch (error) {
      thrown = error;
    }
    assert(thrown && String(thrown.message).includes(prefix) && String(thrown.message).includes('test-fehler'),
      `${workflow.name}: ${exitName} muss die intern formulierte Fehlermeldung werfen.`);

    const success = vm.runInNewContext(`(function () { ${code} })()`, {
      $input: { item: { json: { valid: true, marker: file } } },
      Error,
    });
    assert(success[0].json.status === 'success' && success[0].json.marker === file,
      `${workflow.name}: ${exitName} muss erfolgreiche Daten unveraendert durchreichen.`);
  }
}
...
const fanInRows = [
  {json:{batchId:'test-batch',candidate:'A',status:'success',payload:JSON.stringify({candidate:'A',club:'Club A',contract:'Simulierter Vertrag bis 30.06.2027',marketValue:'5 Mio. EUR',injuries:'Keine bekannten Verletzungen',news:['ok'],source:'source',timestamp:'2026-09-08T00:00:00.000Z',confidence:0.8,uncertain:false}),completedAt:'2026-09-08T00:00:02.000Z'}},
  {json:{batchId:'test-batch',candidate:'B',status:'error',payload:JSON.stringify({candidate:'B',club:null,contract:null,marketValue:null,injuries:null,news:[],source:'Recherche fehlgeschlagen',timestamp:'2026-09-08T00:00:02.100Z',confidence:0,uncertain:true,uncertaintyReason:'simulierter Fehler'}),completedAt:'2026-09-08T00:00:02.100Z'}},
];
...
const fanInResult = vm.runInNewContext(`(function () { ${fanInCode} })()`, {
  ...
  Date, Set, Map, JSON, Number, String, Boolean, Error,
});
...
const testItem = {
  playerSearch: { shortlist: [{ name: 'Analytics Star', score: 95 }, { name: 'Machbarer Kandidat', score: 82 }] },
  research: [
    {candidate:'Analytics Star',contract:futureContract(6),marketValue:'80 Mio. EUR',injuries:'Wiederkehrende schwere Verletzungen',news:['Der Verein bezeichnet den Spieler als unverkäuflich.'],confidence:0.95,uncertain:false},
    {candidate:'Machbarer Kandidat',contract:futureContract(1),marketValue:'5 Mio. EUR',injuries:'Keine bekannten Verletzungen',news:['Der Spieler gilt als wechselbereit und hat Transferfreigabe.'],confidence:0.40,uncertain:false},
  ],
};
const result = vm.runInNewContext(`(function () { ${recommendationCode} })()`, {
  $input: { item: { json: testItem } }, Date, Map, Math, Number, Array, String, RegExp,
});
...
assertEncapsulatedCapabilityErrors();

console.log('OK: Profil-Loop, asynchroner Research-Fan-out/Fan-in, Transfer-Realisierbarkeit und gekapselte Capability-Fehlerbehandlung verifiziert.');
